...
import { MdOutlineAddAPhoto, MdLockReset } from "react-icons/md";
import PasswordChangeModal from "../commonModals/PasswordChangeModal";

const AdminSettings = () => {
  const { user, setUser, islogin, role } = useAuth();
  const [editingProfile, setEditingProfile] = useState(false);
  const [profilePic, setProfilePic] = useState(null);
  const [profilePicPreview, setProfilePicPreview] = useState(null);
  const [isLoading, setIsLoading] = useState(false);
  const [isPasswordChange, setIsPassworddChange] = useState(false);
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            {!editingProfile ? (
              <div className="flex gap-3">
                ...
                <button
                  onClick={() => setIsPassworddChange(true)}
                  className="flex items-center gap-2 bg-(--color-primary-content) text-(--color-primary) px-3 py-1 rounded text-sm hover:bg-(--color-primary) hover:text-(--color-primary-content)"
                >
