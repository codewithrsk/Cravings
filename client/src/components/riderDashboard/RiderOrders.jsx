    ...
    <div className="min-h-screen p-6">
      <div className="max-w-7xl mx-auto">
        <header className="mb-6 flex justify-between items-center">
          ...
          <div className="text-sm font-semibold bg-(--color-base-200) px-3 py-1 rounded">
            Total: {orders.length}
          </div>
        </header>
        ...
        {filteredOrders.length === 0 ? (
          <div className="text-center py-16 bg-(--color-base-200) rounded-xl">
            No deliveries in this section.
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredOrders.map((order) => (
              <div
                key={order.id}
                className="bg-(--color-base-100) rounded-xl p-4 shadow-sm border border-(--color-base-300)"
              >
                <div className="flex justify-between items-start border-b border-(--color-base-300) pb-3 mb-3">
                  ...
                  <span className="text-xs bg-(--color-base-200) px-2 py-1 rounded">
                    {order.time}
                  </span>
                </div>
                  ...
                  <div>
                    {order.status !== "delivered" ? (
                      <button
                        ...
                        className="px-3 py-2 rounded bg-(--color-primary) text-(--color-primary-content)"
                      >
                        Next
                      </button>
                    ) : (
